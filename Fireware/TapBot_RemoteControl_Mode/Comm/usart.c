 ...
 * @author      Clomol
 * @date        2026-2027
 * @brief       各个串口的初始化函数和相关操作函数的声明
 *          -----------------------    --------中断优先级----------                   
 *          | PA9  - USART1(TX)      |      DMA      3  3
 *          | PA10 - USART1(RX)      |			DMA			 
 *          |                        |
 *          | PD5  - USART2(TX)  重  |     	DMA      3  1
 *          | PD6  - USART2(RX)  重  |			DMA
 *          |                        |
 ...
 * @note        串2  串3   串4  已改为DMA接收及发送【date   2022】
 * @warning     
*********************************************************************************/
#include "usart.h"
...
char 	UART4_DMA_Rece_Buf[UART4_MAX_RECV_LEN];     	// USART4 接收数组

char 	UART4_RX_BUF[UART4_MAX_RECV_LEN]; 				// 接收缓冲,最大UART4_MAX_RECV_LEN个字节.
u16 	UART4_RX_STA = 0;                             	// 串口4标志位
int 	LEN4 = 0;										// 串口4数据长度
