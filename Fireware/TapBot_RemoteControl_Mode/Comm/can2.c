/******************** (C) COPYRIGHT 2026 *****************************************
 * @file        can2.c
 * @brief       CAN2 interface initialization and RX interrupt (PB5/PB6).
 *********************************************************************************/
#include "can2.h"
...
void CAN2_RX0_IRQHandler(void)
{
    ...
    if(RxMessage.StdId == WIT_IMU_CAN_ADDR)
    {
        if(RxMessage.IDE == CAN_ID_STD && RxMessage.RTR == CAN_RTR_DATA)
        {
            ...
            WitImu_OnCanFrame(RxMessage.StdId, RxMessage.Data, RxMessage.DLC);
            CntRxCAN2 = 80U;
            CAN2_OutCommunFlg = 0U;
        }
    }
    else if(RxMessage.StdId == 0x00 || RxMessage.StdId == 0x01)  /* 读取数据 */
    {
        if(RxMessage.IDE == CAN_ID_STD && RxMessage.RTR == CAN_RTR_DATA)
        {
            for(i=0;i<RxMessage.DLC;i++)
