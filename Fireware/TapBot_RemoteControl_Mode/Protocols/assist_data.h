...
/* 辅助�?�?主板 信息 */
struct ReceAssist_Msg					//定义 辅助�?�?主板 数据结构�?
{
	u8	RStart1;								// 0  数据帧头 0xA8
	u8  RStart2;								// 1  数据帧头 0x99
	u8 	MsgNumber;					   	// 2  通信确认 0x00-FF的循环数
	
	s8 	LftEngWaterTemp;				// 3  左发动机水温�? 1字节�? INT8，单位： ℃，最低有效位�?  1，[-30 - +100] �?
	u16 LftEngSpeedValue;				// 4  左发动机转速�? 2字节，UINT16，单位：RPM，最低有效位�?  1，[  0 - 5000]RPM 	
	u8 	LftEngThrotValue;				// 6  左主机油门�?   1字节�?UINT8，单位：  %，最低有效位�?  1，[  0 -  100]  %
	u8 	LftEngOilPress;					// 7  左发动机油压�? 1字节�?UINT8，单位：kPa，最低有效位�?  4，[  0 - 1000]kPa
	u8  LftEngineState;					// 8  左发动机状�?   1字节�?UINT8�?
	u16 LftEngFuelRate;		    	// 9  左发动机油�?   2字节，UINT16，单位：L/h，最低有效位�?.05，[0 - 3212.75]L/h

	s8 	RgtEngWaterTemp;				// 11 右发动机水温�? 1字节�? INT8，单位： ℃，最低有效位�?  1，[-30 - +100] �?
	u16	RgtEngSpeedValue;				// 12 右发动机转速�? 2字节，UINT16，单位：RPM，最低有效位�? 1，[  0 - 5000]RPM 	
	u8  RgtEngThrotValue;				// 14 右主机油门�?   1字节�?UINT8，单位：  %，最低有效位�? 1，[  0 -  100]  %
	u8  RgtEngOilPress;					// 15 右发动机油压�? 1字节�?UINT8，单位：kPa，最低有效位�?  4，[  0 - 1000]kPa
	u8  RgtEngineState;					// 16 右发动机状�?   1字节�?UINT8�?
	u16 RgtEngFuelRate;		    	// 17 右发动机油�?   2字节，UINT16，单位：L/h，最低有效位�?.05，[0 - 3212.75]L/h 
	
	u8	LftOilTankVolume;				// 19 左油量反�?		1字节�?UINT8，单位：  %，最低有效位�?  1，[  0 -  100]  %
	u8	RgtOilTankVolume;				// 20 右油量反�?		1字节�?UINT8，单位：  %，最低有效位�?  1，[  0 -  100]  %
	...
	u8	Reserve1;								// 33 预留�?�?			1字节，UINT8
	u8	Reserve2;								// 34 预留�?�?			1字节，UINT8
	u8	SumCheck;    						// 35 校验�?
};
...
/* 主板 �?辅助�?指令 */
struct  SendToAssist_Msg       	//定义底层 主板 �?辅助�? 数据结构�?
{
	...
	u8 	SwitchCmd3;							// 5  开关命�?
																		//bit7-bit6	预留		
																		//bit5-bit4	预留   		
																		//bit3-bit2	管道风机  	11-开 		00-�?   	01 10-无效
...
extern u8 AssistBotMastCtrl;			//可倒桅杆控制�? 01-上升  02-下降  0xFF-无效

extern u8 g_u8OilTankVolume;						//油箱油量
extern u8 g_u8BattPercent_1;						//电池电量 01 底层电池 百分�? 0-100  最小单�?0
extern u8 g_u8BattPercent_2;						//电池电量 02 启动电池 百分�? 0-100  最小单�?0
