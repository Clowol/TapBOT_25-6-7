/******************** (C) COPYRIGHT 2024 *****************************************
 * File Name  : upper_comm.c
 * Description: Parses upper-computer frames from USART2 ring buffer.
 *
 * Frame:
...
void Upper_CommProc(void)
{
	...
	static u8 payload_index = 0U;
	static u8 crc_low = 0U;
	static u8 crc_buf[UPPER_PAYLOAD_MAX_LEN + 2U];
	static u8 payload[UPPER_PAYLOAD_MAX_LEN];
	u8 byte_data;
	u16 crc_calc;
	u16 crc_recv;

	while(USART2_RxRead(&byte_data, 1U) == 1U)
	{
		switch(state)
		{
