...
u8 g_u8BattPercent_1 =80;															//电池电量 01 底层电池 百分比  0-100  
u8 g_u8BattPercent_2 =80;															//电池电量 02 启动电池 百分比  0-100 
...
{	
			...
			0x99,						//	u8  RStart2;								// 1  数据帧头 0x99
			   0,						//	u8 	MsgNumber;					   	// 2  通信确认 0x00-FF的循环数
				 ...
				 0,						//	s8 	LftEngWaterTemp;				// 3  左发动机水温值, 1字节，  INT8，单位： ℃，最低有效位：   1，[-30 - +100] ℃
			   0,						//	u16 LftEngSpeedValue;				// 4  左发动机转速值, 2字节，UINT16，单位：RPM，最低有效位：   1，[  0 - 5000]RPM 	
			   0,						//	u8 LftEngThrotValue;				// 6  左主机油门值,   1字节， UINT8，单位：  %，最低有效位：   1，[  0 -  100]  %
			   0,						//	u8 	LftEngOilPress;					// 7  左发动机油压值, 1字节， UINT8，单位：kPa，最低有效位：   4，[  0 - 1000]kPa
			   0,						//	u8  LftEngineState;					// 8  左发动机状态,   1字节， UINT8， 
			   0,						//	u16 LftEngFuelRate;		    	// 9  左发动机油耗,   2字节，UINT16，单位：L/h，最低有效位：0.05，[0 - 3212.75]L/h

				 0,						//	s8 	RgtEngWaterTemp;				// 11 右发动机水温值, 1字节，  INT8，单位： ℃，最低有效位：   1，[-30 - +100] ℃
			   0,						//	u16	RgtEngSpeedValue;				// 12 右发动机转速值, 2字节，UINT16，单位：RPM，最低有效位：  1，[  0 - 5000]RPM 	
			   0,						//	u8  RgtEngThrotValue;				// 14 右主机油门值,   1字节， UINT8，单位：  %，最低有效位：  1，[  0 -  100]  %
			   0,						//	u8  RgtEngOilPress;					// 15 右发动机油压值, 1字节， UINT8，单位：kPa，最低有效位：   4，[  0 - 1000]kPa
			   0,						//	u8  RgtEngineState;					// 16 右发动机状态,   1字节， UINT8， 
			   0,						//	u16 RgtEngFuelRate;		    	// 17 右发动机油耗,   2字节，UINT16，单位：L/h，最低有效位：0.05，[0 - 3212.75]L/h 

	
			  81,						//	u8	LftOilTankVolume;				// 19 左油量反馈			1字节， UINT8，单位：  %，最低有效位：   1，[  0 -  100]  %
			  82,						//	u8	RgtOilTankVolume;				// 20 右油量反馈			1字节， UINT8，单位：  %，最低有效位：   1，[  0 -  100]  %
		...
		(12.6-5)*10,			//	u8	BatteryVol_6;						// 26 电池6 	1字节， UINT8，单位：  V，最低有效位： 0.1，[  5 - 30.3]  V		0V=5V
			   ...
			   0,						//	u8	Reserve1;								// 33 预留位1，				1字节，UINT8
			   0,						//	u8	Reserve2;								// 34 预留位2，				1字节，UINT8
			   0,						//	u8	SumCheck;    						// 35 校验和
};


struct SendToAssist_Msg	SendToAssistMsg=			  //定义发送的数据结构体
{	
			0xA8,						//	u8 	SStart1;								// 0  数据帧头 0xA8
			0x99,						//	u8 	SStart2;								// 1  数据帧头 0x99	
			   0,						//	u8 	MsgNumber;	           	// 2  通信确认
			   0,						//	u8 	SwitchCmd1;							// 3  开关命令1	
...
//		swgPrt( "RAssitOk \r\n "); 
		...
		ReceAssistMsg.MsgNumber=ReceDat[2];	 								//接收数据字节作为高字节
		
		ReceAssistMsg.LftEngWaterTemp=ReceDat[3];	 					//接收数据字节作为高字节	
		
		ReceAssistMsg.LftEngSpeedValue=ReceDat[5];	 				//接收数据字节作为高字节
		ReceAssistMsg.LftEngSpeedValue<<=8;			 						//高字节左移8位
		ReceAssistMsg.LftEngSpeedValue|=ReceDat[4];	 				//接收数据字节作为低字节
		
		ReceAssistMsg.LftEngThrotValue=ReceDat[6];	 				//接收数据字节作为高字节	
		ReceAssistMsg.LftEngOilPress=ReceDat[7];	 					//接收数据字节作为低字节
		ReceAssistMsg.LftEngineState=ReceDat[8];	 					//接收数据字节作为高字节	

		ReceAssistMsg.LftEngFuelRate=ReceDat[10];	 					//接收数据字节作为高字节
		ReceAssistMsg.LftEngFuelRate<<=8;			 							//高字节左移8位
		ReceAssistMsg.LftEngFuelRate|=ReceDat[9];	 					//接收数据字节作为低字节

		ReceAssistMsg.RgtEngWaterTemp=ReceDat[11];	 				//接收数据字节作为高字节	

		ReceAssistMsg.RgtEngSpeedValue=ReceDat[13]; 				//接收数据字节作为高字节
		ReceAssistMsg.RgtEngSpeedValue<<=8;			 						//高字节左移8位
		ReceAssistMsg.RgtEngSpeedValue|=ReceDat[12];	 			//接收数据字节作为低字节
		
		ReceAssistMsg.RgtEngThrotValue=ReceDat[14]; 				//接收数据字节作为高字节
		ReceAssistMsg.RgtEngOilPress=ReceDat[15];	 					//接收数据字节作为低字节
		ReceAssistMsg.RgtEngineState=ReceDat[16];	 					//接收数据字节作为高字节	

		ReceAssistMsg.RgtEngFuelRate=ReceDat[18]; 					//接收数据字节作为高字节
		ReceAssistMsg.RgtEngFuelRate<<=8;			 							//高字节左移8位
		ReceAssistMsg.RgtEngFuelRate|=ReceDat[17];	 				//接收数据字节作为低字节
		...
		ReceAssistMsg.DeviceState=ReceDat[28];	 						//接收数据字节作为高字节
		ReceAssistMsg.DeviceState<<=8;			 								//高字节左移8位
		ReceAssistMsg.DeviceState|=ReceDat[27];	 						//接收数据字节作为低字节
		
		ReceAssistMsg.TempData=ReceDat[30];	 								//接收数据字节作为高字节
		ReceAssistMsg.TempData<<=8;			 										//高字节左移8位
		ReceAssistMsg.TempData|=ReceDat[29];	 							//接收数据字节作为低字节

		ReceAssistMsg.HumiData=ReceDat[32];	 								//接收数据字节作为高字节
		ReceAssistMsg.HumiData<<=8;			 										//高字节左移8位
		ReceAssistMsg.HumiData|=ReceDat[31];	 							//接收数据字节作为低字节
...
{	
	...
	if(g_RmtUpManRealMode == RMT_MODE)
	{
		if(Rmtrece_Msg.CH_LD <= 300)  
			SendToAssistMsg.SwitchCmd1 |= (0x03<<0);			//开启			
