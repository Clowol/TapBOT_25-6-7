...
/**************************************************************************
  ...
  * @note   
***********************************************************************/
u16 flag10ms = 0;						// 10ms 时间片标志
u16 CntRx1 = 0;		  				// 串口1 接收时间标志位
u16 CntRx2 = 0;		  				// 串口2 接收时间标志位
u16 CntRx3 = 0;		  				// 串口3 接收时间标志位
u16 CntRx4 = 0;		  				// 串口4 接收时间标志位
u16 CntRx5 = 0;		  				// 串口5 接收时间标志位

u16 CntRxUp 	 = 60;				// 上层通信 接收帧计数
u16 CntRxAssist = 20;				// 辅助控制板通信 接收帧计数 
u16 CntRxRmt 	 = 80;				// 遥控器通信 接收帧计数
u16 CntRxCAN1 	 = 80;				// CAN1通信 接收帧计数
u16 CntRxCAN2 	 = 80;				// CAN2通信 接收帧计数

u8 UpOutCommunFlg 	= 1;			//上层通信失联	 标志位   	0-正常  1-失联
u8 AssistOutCommunFlg = 1;			//辅助控制板失联 标志位  	0-正常  1-失联
u8 RmtOutCommunFlg = 1;				//遥控器通信失联 标志位		0-正常  1-失联
u8 CAN1_OutCommunFlg = 1;			//CAN1通信失联	 标志位		0-正常  1-失联
u8 CAN2_OutCommunFlg = 1;			//CAN2通信失联	 标志位		0-正常  1-失联
u8 RmtPwrOffFlg = 1;					//遥控器掉电	 标志位		0-正常  1-失联
